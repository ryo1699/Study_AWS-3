...
import { Download, FileSpreadsheet, ImageUp, RefreshCw, Save, Trash2 } from "lucide-react";
import "./styles.css";
...
function App() {
  ...
  async function createCsvExport() {
    const job = await request<CsvExportJob>("/api/csv-exports", {
      method: "POST",
      body: JSON.stringify({})
    });
    setCsvJob(job);
    setMessage(`CSV export job #${job.id} created`);
  }

  async function refreshCsvExport() {
    if (!csvJob) return;
    const job = await request<CsvExportJob>("/api/csv-exports/" + csvJob.id);
    setCsvJob(job);
    setMessage(`CSV export job #${job.id}: ${job.status}`);
  }

  async function downloadCsv() {
    if (!csvJob) return;
    const response = await request<{ downloadUrl: string }>("/api/csv-exports/" + csvJob.id + "/download-url", {
      method: "POST",
      body: JSON.stringify({})
    });
  ...
  return (
    <main className="app-shell">
      ...
      <section className="grid">
        ...
        <section className="panel">
          <h2>CSV export</h2>
          <button className="primary" type="button" onClick={() => createCsvExport().catch((error) => setMessage(error.message))}>
            <FileSpreadsheet size={18} />
            Export CSV
          </button>
          {csvJob && (
            <div className="csv-job">
              <strong>Job #{csvJob.id}</strong>
              <span className={"status status--" + (csvJob.status === "complete" ? "done" : csvJob.status === "failed" ? "failed" : "in_progress")}>
                {csvJob.status}
              </span>
              {csvJob.errorMessage && <p>{csvJob.errorMessage}</p>}
              <div className="task-card__actions">
                <button type="button" onClick={() => refreshCsvExport().catch((error) => setMessage(error.message))}>
                  <RefreshCw size={18} />
                  Refresh
                </button>
                <button type="button" disabled={csvJob.status !== "complete"} onClick={() => downloadCsv().catch((error) => setMessage(error.message))}>
                  <Download size={18} />
                  Download
                </button>
              </div>
